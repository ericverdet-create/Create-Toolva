...
// MD5 — simple implementation (no external deps)
...
export default function HashGenerator() {
  ...
  useEffect(() => {
    ...
    setHashes({ MD5: md5(input) });
    Promise.all([
      sha('SHA-1', input),
      sha('SHA-256', input),
      sha('SHA-512', input),
    ]).then(([s1, s256, s512]) => {
      setHashes({ MD5: md5(input), 'SHA-1': s1, 'SHA-256': s256, 'SHA-512': s512 });
    });
  }, [input]);

  const copyHash = async (h: string) => { await navigator.clipboard.writeText(h); };

  return (
    <div className="space-y-5">
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Texto a hashear</label>
        <textarea value={input} onChange={e => setInput(e.target.value)} rows={3} placeholder="Escribe o pega el texto aquí..."
          className="w-full border border-gray-300 dark:border-gray-600 rounded-xl px-4 py-2.5 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-400 focus:outline-none text-sm resize-none" />
      </div>

      {Object.keys(hashes).length > 0 && (
        <div className="space-y-3">
          {Object.entries(hashes).map(([alg, hash]) => (
            <div key={alg} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-semibold text-indigo-600 uppercase tracking-wide">{alg}</span>
                <button onClick={() => copyHash(hash)}
                  className="text-xs text-gray-500 hover:text-indigo-600 transition-colors px-2 py-0.5 rounded border border-gray-200 dark:border-gray-600 hover:border-indigo-300">
                  Copiar
                </button>
              </div>
      ...
      {!input && (
        <p className="text-sm text-gray-400 text-center py-4">Escribe algo para generar el hash</p>
      )}
    </div>
  );
}
